...
def _config(sender, keyword, user_data):
    ...
    if isinstance(sender, dcg.RadioButton): # == "mvAppItemType::mvRadioButton":
        value = True
    else:
...
def _hsv_to_rgb(h, s, v):
    if s == 0.0: return (int(255*v), int(255*v), int(255*v))
...
def _create_static_textures():
    ...
    holder.append(dpg.add_static_texture(100, 100, texture_data1, parent="__demo_texture_container", tag="__demo_static_texture_1", label="Static Texture 1"))
    holder.append(dpg.add_static_texture(50, 50, texture_data2, parent="__demo_texture_container", tag="__demo_static_texture_2", label="Static Texture 2"))
    holder.append(dpg.add_static_texture(100, 100, texture_data3, parent="__demo_texture_container", tag="__demo_static_texture_3", label="Static Texture 3"))
...
def _create_dynamic_textures():
    ...
    holder.append(dpg.add_dynamic_texture(100, 100, texture_data1, parent="__demo_texture_container", tag="__demo_dynamic_texture_1"))
    holder.append(dpg.add_dynamic_texture(50, 50, texture_data2, parent="__demo_texture_container", tag="__demo_dynamic_texture_2"))
    ...
    with dpg.window(label="Dear PyGui Demo", width=800, height=800, on_close=_on_demo_close, pos=(100, 100), tag="__demo_id"):
        ...
        with dpg.menu_bar():
            ...
            with dpg.menu(label="Menu"):
                ...
                with dpg.menu(label="Settings"):
                    ...
                    with dpg.child_window(height=60, auto_resize_x=True, delay_search=True):
                        for i in range(10):
            ...
            with dpg.tree_node(label="Combo"):
                ...
                def change_combo_height(sender, app_data, user_data):
                    ...
                    if app_data == "mvComboHeight_Small":
                        dpg.configure_item("combo_demo", height_mode="small")
                    elif app_data == "mvComboHeight_Regular":
                        dpg.configure_item("combo_demo", height_mode="regular")
                    elif app_data == "mvComboHeight_Large":
                        dpg.configure_item("combo_demo", height_mode="large")
                    elif app_data == "mvComboHeight_Largest":
                        dpg.configure_item("combo_demo", height_mode="largest")
        ...
        with dpg.collapsing_header(label="Layout & Scrolling"):
            ...
            with dpg.tree_node(label="Containers"):
                ...
                with dpg.tree_node(label="Child windows"):
                    ...
                    with dpg.group(horizontal=True):
                        dpg.add_checkbox(label="auto_resize_x", callback=_config, user_data=demo_layout_child)
                        dpg.add_checkbox(label="auto_resize_y", callback=_config, user_data=demo_layout_child)
                    ...
                    with dpg.child_window(auto_resize_x=True, height=130, menubar=True):
                        with dpg.menu_bar():
        ...
        with dpg.collapsing_header(label="Inputs & Events"):
            ...
            with dpg.tree_node(label="Global"):
                ...
                kh_down = dpg.add_text("Key Down Handler:", label="Key id:  seconds:", show_label=True)
                kh_release = dpg.add_text("Key Release Handler:", label="Key id:", show_label=True)
                kh_press = dpg.add_text("Key Press Handler:", label="Key id:", show_label=True)
                mh_click = dpg.add_text("Mouse Click Handler", label="Mouse id:", show_label=True)
                mh_double = dpg.add_text("Mouse Double Click Handler", label="Mouse id:", show_label=True)
                mh_down = dpg.add_text("Mouse Down Handler", label="Mouse id:  seconds:", show_label=True)
                mh_release = dpg.add_text("Mouse Release Handler", label="Mouse id:", show_label=True)
                mh_wheel = dpg.add_text("Mouse Wheel Handler", label="Mouse id:", show_label=True)
                mh_move = dpg.add_text("Mouse Move Handler", label="Mouse pos:", show_label=True)
                mh_drag = dpg.add_text("Mouse Drag Handler", label="Mouse id:  delta:", show_label=True)
            ...
            def _event_handler(sender, data):
                if isinstance(sender, dpg.dcg.KeyDownHandler):
                    kh_down.label = f"Key id: {data[0]}, Seconds:{data[1]}"
                elif isinstance(sender, dpg.dcg.KeyReleaseHandler):
                    kh_release.label = f"Key id: {data}"
                elif isinstance(sender, dpg.dcg.KeyPressHandler):
                    kh_press.label = f"Key id: {data} + Shift: {dpg.is_key_down(dpg.mvKey_LShift)}"
                elif isinstance(sender, dpg.dcg.MouseClickHandler):
                     mh_click.label = f"Mouse id: {data} + Shift: {dpg.is_key_down(dpg.mvKey_LShift)}"
                elif isinstance(sender, dpg.dcg.MouseDoubleClickHandler):
                    mh_double.label = f"Mouse id: {data}"
                elif isinstance(sender, dpg.dcg.MouseDownHandler):
                    mh_down.label = f"Mouse id: {data[0]}, Seconds:{data[1]}"
                elif isinstance(sender, dpg.dcg.MouseReleaseHandler):
                    mh_release.label = f"Mouse id: {data}"
                elif isinstance(sender, dpg.dcg.MouseWheelHandler):
                    mh_wheel.label = f"Mouse id: {data}"
                elif isinstance(sender, dpg.dcg.MouseMoveHandler):
                    mh_move.label = f"Mouse pos: {data}"
                elif isinstance(sender, dpg.dcg.MouseDragHandler):
                    mh_drag.label = f"Mouse id: {data[0]}, Delta:{[data[1], data[2]]}"
